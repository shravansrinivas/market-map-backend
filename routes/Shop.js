...
router.post("/:cityName/:category/:subcategory", async (req, res) => {
  Shops.findOne({shopName:req.body.shopName,    category: req.params.category,
    subcategory: req.params.subcategory,
    cityName: req.params.cityName,
    address: req.body.address,
    latitude: req.body.latitude,
    longitude: req.body.longitude}).then(async(data)=>{
      if(data==null){
...
router.post("/byUser/:cityName/:category/:subcategory", async (req, res) => {
  ...
  Shops.findOne({shopName:req.body.shopName,    category: req.params.category,
    ...
    address: req.body.address,
    latitude: latitude,
    longitude: longitude}).then(async(data)=>{
      if(data==null){
  const shopToSave = new Shops({
    shopName: req.body.shopName,
    category: req.params.category,
...
router.post("/v2/byUser/:cityName/:category/:subcategory", async (req, res) => {
  ...
  .get(
    `${process.env.MAPQUEST_API_BASE}${
      process.env.MAPQUEST_API_KEY
    }&street=${req.body.streetName}&city=${req.params.cityName}&state=${state}&postalCode=${req.body.zip}&adminArea1=IN`
  ).then(async (response) => {
    latitude = response.data.results[0]["locations"][0]["latLng"]["lat"];
    longitude = response.data.results[0]["locations"][0]["latLng"]["lng"];
  }).then(()=>{
    Shops.findOne({shopName:req.body.shopName,    
      category: req.params.category,
      subcategory: req.params.subcategory,
      cityName: req.params.cityName,
...
router.put("/:cityName/byCategories", async (req, res) => {
  ...
  if(req.body.categories!==undefined || req.body.categories){filter['category']= {$in: req.body.categories}}
...
router.delete("/:city/oneshop", async (req, res) => {
  const shops = await Shops.deleteOne({shopName:req.body.shopName, cityName:req.params.city})
...
router.delete("/:city/many", async (req, res) => {
  const shops = await Shops.deleteMany({shopName:req.body.shopName, cityName:req.params.city})
    .then((data) => res.json({ message: "Shop record deleted" }))
    .catch((err) => {
      res.json({ errorMessage: err });
    });
...
router.delete("/:cityName/:subcategory", async (req, res) => {
  const shops = await Shops.deleteMany({cityName:req.params.cityName, subcategory:req.params.subcategory})
    .then((data) => res.json({ message: "All records deleted" }))
    .catch((err) => {
      res.json({ errorMessage: err });
    });
